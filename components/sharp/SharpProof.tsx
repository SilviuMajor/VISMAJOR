      ...
      {Array.from({ length: SEGMENTS }).map((_, idx) => {
        ...
        return (
          <motion.span
            ...
            style={{ background: filled ? "var(--ink-0)" : "rgba(20,19,15,0.18)" }}
...
export function SharpProof() {
  ...
  return (
    <section
      id="proof"
      className="overflow-hidden border-y bg-paper-0 py-14 md:py-20"
      style={{ borderColor: "rgba(20,19,15,0.14)" }}
    >
      <Container>
        {/* Masthead */}
        <Reveal>
          <div className="flex items-center justify-between gap-4">
            <span className="caps-loose text-[11px] font-medium text-ink-0">
              The First-Batch List
            </span>
            <span className="flex items-center gap-2">
              <span className="relative flex h-1.5 w-1.5">
                {!reduce && (
                  <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-ink-0/60" />
                )}
                <span className="relative inline-flex h-1.5 w-1.5 rounded-full bg-ink-0" />
              </span>
              <span className="caps text-[10px] font-medium text-ink-0/70">
                Early-bird open
              </span>
            </span>
          </div>
        ...
        <Reveal delay={0.05}>
          <div
            ...
            style={{ borderColor: "rgba(20,19,15,0.28)" }}
          >
            <div>
              <div className="flex items-end gap-3 md:gap-4">
                <span
                  className="stat-tab font-semibold text-ink-0"
                  style={{
              ...
              <p className="mt-4 caps text-[11px] font-medium text-ink-0">
                The one they reach for at 7am.
              </p>
            </div>

            <p className="max-w-md pb-1 text-[16.5px] leading-[1.6] text-ink-0 lg:justify-self-end">
              ...
              <span className="font-mono text-ink-0/45 line-through">£30</span> at launch
        ...
        <Reveal delay={0.05}>
          <div className="mt-10">
            <div className="flex items-end justify-between">
              <span className="caps text-[11px] font-medium text-ink-0">
              ...
              <span className="flex items-baseline gap-2">
                ...
                <span className="caps text-[10.5px] font-medium text-ink-0">reserved</span>
              </span>
            </div>
            <div className="mt-4">
              <AllocationMeter />
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
              <span className="caps text-[10.5px] font-medium text-ink-0/70">
        ...
        <Reveal delay={0.05}>
          ...
          >
            <div
              ...
              style={{ background: "linear-gradient(to right, var(--paper-0), transparent)" }}
              aria-hidden
            />
            <div
              className="pointer-events-none absolute inset-y-0 right-0 z-10 w-16 md:w-28"
              style={{ background: "linear-gradient(to left, var(--paper-0), transparent)" }}
              aria-hidden
            />
                ...
                <div className="marquee-group" key={group} aria-hidden={group === 1}>
                  {marqueeQuotes.map((qt, i) => (
                    <figure
                      ...
                      style={{ borderColor: "rgba(20,19,15,0.12)" }}
                    >
                      <blockquote className="text-[16.5px] leading-[1.5] text-ink-0">
                        &ldquo;{qt.q}&rdquo;
                      </blockquote>
                      <figcaption className="caps text-[10.5px] font-medium text-ink-0/70">
                        {qt.a}
                      </figcaption>
                    </figure>
                  ))}
        ...
        <Reveal delay={0.05}>
          <ul className="mt-8 flex flex-wrap items-center gap-x-7 gap-y-3">
            {TRUST.map((t) => (
              <li
                key={t}
                className="caps inline-flex items-center gap-2 text-[10px] font-medium text-ink-0/70"
              >
                <span className="inline-block h-1 w-1 rounded-full bg-ink-0/45" />
                {t}
              </li>
            ))}
          </ul>
